...
def convert_odf_timetable2_c_header(
            input_odf_timetable):
    name,weeks = extract_odf_data(input_odf_timetable)
    return data_to_header(name, weeks)

def extract_odf_data(input_odf_timetable):
    data = pd.read_excel(input_odf_timetable, engine="odf")
    name = data.columns[0]
